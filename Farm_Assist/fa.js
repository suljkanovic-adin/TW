...
function createOptionWindow() {
    // Check if the option window already exists
    if ($("#farmOptionWindow").length === 0) {
        // Create the option window
        var optionWindow = $("<div id='farmOptionWindow' style='position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 20px; border: 2px solid #333;'></div>");
        optionWindow.append("<label><input type='radio' name='farmOption' value='a' checked> Option A</label><br>");
        optionWindow.append("<label><input type='radio' name='farmOption' value='b'> Option B</label><br>");
        optionWindow.append("<label><input type='radio' name='farmOption' value='c'> Option C</label><br>");
        optionWindow.append("<button onclick='closeOptionWindow()'>Close</button>");
    
        // Append the option window to the body
        $("body").append(optionWindow);
    }
}

function closeOptionWindow() {
    $("#farmOptionWindow").remove();
}
